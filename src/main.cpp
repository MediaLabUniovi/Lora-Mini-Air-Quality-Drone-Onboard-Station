...
#define PA_ATM 101300

bool ENABLE_DEBUG = true;                                                   // Boolean toggle to enable serial monitor
uint32_t appTxDutyCycle = 10000;                                            // Data transmission rate time (in milliseconds)
...
// Sensors variables -----------------------------------------------------------------------------------------
int temperature, humidity, pressure_int, pressure_dec, batteryVoltage;
long pressure_pa;
float pressure_atm;
...
void pressure_parts(){
  ...
  pressure_atm = pressure_pa / PA_ATM;                                      // Pressure in atm (too many decimals)
...
static void prepareTxFrame(uint8_t port){
  ...
  appData[9] = lowByte(co2_medio);
  appData[10] = highByte(co2_medio);

  appData[11] = lowByte(tvoc_medio);
  appData[12] = highByte(tvoc_medio);
